...
#
# Output:
#   dist\TradingAgentsHelper.exe          portable, double-clickable
#   dist\TradingAgentsHelperSetup.exe     installer (only if Inno Setup's iscc
#                                         is on PATH: winget install JRSoftware.InnoSetup)
#   dist\TradingAgentsHelper-windows.zip  the portable exe, zipped for the
...
if ($iscc) {
  ...
  Write-Host "built: dist\TradingAgentsHelperSetup.exe (installer, v$version)"
} else {
  Write-Host "Inno Setup (iscc) not on PATH - skipped the installer, portable exe only."
}

Compress-Archive -Force -Path dist\TradingAgentsHelper.exe `
  -DestinationPath dist\TradingAgentsHelper-windows.zip
Write-Host "built: dist\TradingAgentsHelper.exe (+ -windows.zip for the download endpoint)"
